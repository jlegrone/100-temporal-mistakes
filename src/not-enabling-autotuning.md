...
> [!TIP]
> * Temporal SDKs offer autotuning (preview) that automatically adjusts worker concurrency settings based on system resource utilization.
...
- **Overprovisioning**: Aggressive settings risk overwhelming the host. If you set `MaxConcurrentActivities` too high and those activities are CPU-intensive, the worker can become unresponsive, miss heartbeats, and cause cascading failures.
...
- **Operational burden**: Manually tuning workers across many task queues and activity types is tedious. Teams often skip it, leaving suboptimal defaults in place.
