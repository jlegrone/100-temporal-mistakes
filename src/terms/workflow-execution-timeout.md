...
The workflow execution timeout limits the total time for a workflow execution, including all runs in a continue-as-new chain. When reached, the workflow is terminated (not cancelled) -- no cleanup code runs. The default is effectively 10 years if not set.
