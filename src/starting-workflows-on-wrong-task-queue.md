...
> [!TIP]
> * Task queues are the link between workflow/activity starters and the workers that execute them. A mismatch means tasks sit indefinitely with no one to pick them up.
> * There are no errors when this happens -- the workflow simply waits forever, making it hard to notice without monitoring.
> * Always validate task queue names through shared constants or configuration, and monitor Schedule-To-Start latency to catch mismatches early.

## What?

Task queues are named queues that Temporal uses to route workflow tasks and activity tasks to the correct workers. When you start a workflow, you specify which task queue it should be placed on. Workers poll specific task queues for work. If these don't match -- due to a typo, a misconfiguration, or a deployment targeting a different service -- the workflow task will sit in the queue with no worker to pick it up.
...
This is one of the most common "silent failures" when working with Temporal. It manifests as workflows that appear to have started successfully (the `StartWorkflow` call returns a valid workflow ID and run ID) but never make any progress.
