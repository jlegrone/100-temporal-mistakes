...
Without early detection, non-determinism errors only surface when a workflow actually replays -- typically after a [worker](terms/worker.md) restart or deployment. By that point the workflow is stuck, the error message may be cryptic, and all in-flight workflows running the affected code are potentially impacted.
