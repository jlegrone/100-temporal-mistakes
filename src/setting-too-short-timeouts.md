# Setting Too-Short Activity Timeouts

<!-- TODO: Break this up into a small sub-section for each timeout type. -->
...
Consider: your activity calls a payment service (normal p99 latency of 80ms), with a 30-second [schedule-to-close timeout](terms/schedule-to-close-timeout.md). But a temporary service disruption causes 3 minutes of intermittent failures. The activity retries a few times, hits the timeout, and fails permanently. With a more generous timeout, the same scenario plays out differently: retries succeed after the outage window.

<!-- TODO: Add note on avoiding too-short start to close timeouts (must not be shorter than what the business logic actually requires) -->
...
<!-- TODO: Add code example for a variable time data processing activity which may enter a livelock due to a too-short start to close timeout. -->