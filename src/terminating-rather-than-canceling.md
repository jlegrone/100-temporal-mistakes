...
When operators need to stop a running workflow, Temporal offers two mechanisms: **terminate** and **cancel**. Many teams default to termination because it feels decisive and they see it first in the UI or CLI, but this denies the workflow any opportunity to clean up.
...
Cancellation is cooperative. Temporal delivers a cancellation request to the workflow, which can then catch it, run compensation logic (rolling back transactions, releasing external resources, notifying downstream systems), and complete gracefully.

## Why?

Reaching for terminate when cancel would suffice is dangerous for several reasons:

- **No cleanup**: Activities that allocated external resources (database locks, cloud infrastructure, third-party reservations) won't release them. You're left with leaked resources that require manual intervention.
...
- **Lost observability**: A terminated workflow's final state tells you almost nothing about what was happening. A cancelled workflow can log its cleanup steps and complete with a meaningful result or error.
...
2. **Design workflows to handle cancellation** by wrapping cleanup logic in a deferred cancellation scope. In Go, use `workflow.NewDisconnectedCtx` and handle the `CanceledError`. In TypeScript, use `CancellationScope` with cleanup handlers.
