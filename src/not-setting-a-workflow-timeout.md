...
> [!TIP]
> * Without an execution timeout, a workflow runs until it completes, fails, or is manually [terminated](terms/terminate.md) -- the default is effectively 10 years.
...
**Bugs in workflow logic.** A conditional branch that never triggers, a loop that never exits, a signal wait that never receives its signal. Your workflow sits there forever, occupying space in the server's persistence layer and potentially holding external resources.
...
**Use workflow run timeout for continue-as-new chains.** If your workflow uses [continue-as-new](terms/continue-as-new.md), the execution timeout applies to the entire chain. The run timeout applies to each individual run. Set both: run timeout to catch a single stuck run, execution timeout to catch an endlessly looping chain.
