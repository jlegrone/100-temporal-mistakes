...
During deployments or scaling events, workers are stopped and replaced. If a worker is killed immediately, any activities executing on it are abandoned. The Temporal server won't know the activity failed until its heartbeat or start-to-close timeout expires. Then the activity retries from scratch on another worker, throwing away all completed work.

This is especially painful for long-running activities (data migrations, file processing, ML training jobs) where losing 10-15 minutes of progress per deployment adds up fast.

## Why?

Temporal workers handle two types of tasks: [workflow tasks](terms/workflow-task.md) and activity tasks. Workflow tasks are safe to interrupt because workflows are deterministic and simply [replay](terms/replay.md) from history on the next worker. Activity tasks, however, represent actual side-effecting work. When an activity is interrupted mid-execution, the only recovery mechanism is retrying from the beginning.

Without a drain period:
- Activities time out, adding latency equal to the timeout duration before they retry.
- Completed work is lost and must be redone.
...
**Configure graceful shutdown on your worker.** Most Temporal SDKs support a graceful shutdown period. When the worker receives SIGTERM, it stops polling for new tasks but continues executing in-flight activities until the drain period expires.
...
**Use heartbeats for long-running activities.** Activities that heartbeat detect cancellation (triggered by the worker shutting down) and save their progress. On retry, the activity resumes from the last heartbeat rather than starting over.
