...
> [!TIP]
> Temporal's batch operations API can terminate, [cancel](terms/cancellation.md), [signal](terms/signals.md), or reset many workflows matching a [visibility](terms/visibility.md) query in a single request. Use it instead of writing scripts that iterate over workflows one by one.
