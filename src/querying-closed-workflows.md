...
> [!TIP]
> * [Queries](terms/queries.md) execute against a workflow's current state by replaying its history on a worker.
...
2. **Use search attributes.** Temporal search attributes are stored on the server and queryable via visibility APIs without replaying the workflow. For simple state like status, progress percentage, or business identifiers, search attributes are a better fit than queries.
...
4. **Set retention policies.** Configure namespace retention periods to automatically clean up old workflow histories. This limits how far back queries can reach, which also limits how much versioning baggage you need to carry.
