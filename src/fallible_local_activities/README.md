...
Local activities execute within a workflow task, which has a default 10-second timeout. When a local activity fails and retries with backoff, each attempt eats into that budget. If retries exceed the timeout, the workflow task times out, gets rescheduled, [replays](../terms/replay.md), hits the local activity again, and the retry cycle restarts from zero. The local activity never gets enough time to finish execution.
