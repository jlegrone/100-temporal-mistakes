...
> [!TIP]
> Activities can be [cancelled](terms/cancelation.md) for many reasons -- [heartbeat timeout](terms/heartbeat-timeout.md), [start-to-close timeout](terms/start-to-close-timeout.md), [worker](terms/worker.md) shutdown, or explicit cancelation from the workflow -- not just workflow cancelation.

When an activity's context is cancelled, it's tempting to assume the parent workflow was cancelled. But activities can be interrupted for reasons unrelated to the workflow's lifecycle. If your cancelation handler assumes the workflow is done and acts accordingly, you'll introduce subtle bugs. The workflow may still be running and waiting for the activity to report back.
