...
1. **Deterministic workflow IDs**: If you need to interact with a specific workflow, derive its ID deterministically from your domain data (e.g., `order-{orderID}`, `user-{userID}-subscription`). Then you can signal, [query](terms/queries.md), or describe it directly by ID without searching.
