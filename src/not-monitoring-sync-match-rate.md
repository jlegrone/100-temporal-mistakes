...
> [!TIP]
> * Sync match occurs when a task is immediately dispatched to a waiting [worker](terms/worker.md) poller without being persisted to the database.
> * A high sync match rate means low task delivery latency and reduced load on the persistence layer.
> * A dropping sync match rate is an early warning signal that workers are falling behind -- often before [Schedule-To-Start Latency](terms/schedule-to-start-latency.md) visibly spikes.
...
3. **Diagnose drops** by checking:
   ...
   - **Worker saturation**: Workers that are fully busy executing activities or [workflow tasks](terms/workflow-task.md) won't have idle poller slots. Scale up the number of worker instances.
