...
> [!TIP]
> * Schedule-To-Start Latency (STSL) measures the time between a task being scheduled and a worker picking it up.
...
Schedule-To-Start Latency is the delay between the moment Temporal schedules a task (workflow task or activity task) and the moment a worker actually starts executing it. In a healthy system, this latency is near zero because a worker is almost always available to pick up new tasks immediately. When STSL starts climbing, it means tasks are sitting in a queue waiting for a worker to become available.
...
- **Cascading effects** can occur. As tasks pile up, workers may start timing out on heartbeats for running activities, causing retries that create even more tasks in the queue.
...
2. **Diagnose the root cause** when STSL rises:
   ...
   - Task queue imbalance: if you use multiple task queues, check whether load is unevenly distributed.
