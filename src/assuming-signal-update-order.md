...
Developers often assume that if they send signal A before signal B from the same client, the workflow will process A before B. This assumption is incorrect. [Signals](terms/signals.md) and [updates](terms/updates.md) are asynchronous by nature, and Temporal does not guarantee ordering of delivery across multiple signal or update calls.
...
- **Network conditions.** Different requests may take different paths through load balancers and network infrastructure, arriving at the server in a different order than they were sent.
...
- **Retries.** If a signal delivery fails and is retried by the client, the retry may arrive after a subsequent signal that was sent later.
...
3. **State machines.** Model your workflow state as a state machine that validates transitions. Reject or buffer messages that aren't valid for the current state rather than assuming they'll arrive in the right state.
