...
<!-- TODO: document/add code example for pattern of combining a chain of operations that together are idempotent into one activity. Could be a batch operation or a sequence of changes to a resource like applying a change and waiting for a reconciliation loop in a deployment use case. -->

> [!TIP]
> Every activity creates [history](terms/event-history.md) events, requires serialization, and adds a round-trip to the server. Combine idempotent operations into a single activity if the resulting activity is itself idempotent and cheap to retry.
...
<!-- TODO: Note the loss of observability (coming from the separate entries in workflow history) that can arise as a downside of combining activities into one. -->
<!-- TODO: Can activities read out the error from the last attempt to adjust their own behavior during a retry? Or is that only possible with heartbeat details? -->
<!-- TODO: Cross-link to not using heartbeat details mistake entry. -->
