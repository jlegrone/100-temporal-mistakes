...
Spread events across multiple workflows. Avoid funneling high-throughput work through a single workflow -- don't use one workflow as a [message queue](wrapping-a-queue-with-a-workflow.md) or build [naive batch processing](naive-batch-processing-implementation.md) that concentrates all work in one place.
