...
> [!TIP]
> Individual workflow or activity request/response [payloads](terms/payload.md), signals and updates cannot exceed 4MB by default.

<!-- TODO: Add a note about how workflow commands are combined into one gRPC request sometimes, so for example starting three activities in parallel could result in a >4MB Temporal API payload even if each activity request object is smaller than that. -->
This limititation is inherited from the Tempoeral server which has a gRPC API with a default message size limit of 4MB.
...
<!-- TODO: add an integration test asserting that we see resource exhaustion errors when attempting to schedule three parallel activities each with a 3MB request payload, or if simpler when starting a workflow with large input. -->
<!-- TODO: Link to external payloads documentation. -->
